...
def data_generator(dataset, config, shuffle=True, augment=False, augmentation=None,
                   ...
                   no_augmentation_sources=None, output_names=None):
    ...
    while True:
        try:
            ...
            if dataset.image_info[image_id]['source'] in no_augmentation_sources:
                ...
                image, image_meta, gt_class_ids, gt_boxes, gt_masks = \
                    load_image_gt(dataset, config, image_id, augment=augment,
                                  ...
                                  use_mini_mask=config.USE_MINI_MASK)
            if isinstance(dataset.image_info[image_id]['weight'], (int, float)):
                weight = dataset.image_info[image_id]['weight']
            else:
                weight = None
            ...
            if b == 0:
                ...
                if weight != None:
                    batch_weights = np.zeros(
                        (batch_size,), dtype=np.float32)
                else:
                    batch_weights = np.ones(
                        (batch_size,), dtype=np.float32)
                if random_rois:
            ...
            if b >= batch_size:
                ...
                outputs = []
                weights = {name: batch_weights for name in output_names}
...
class MaskRCNN():
    ...
    def detect_dataset(self, dataset, verbose=0):
        ...
        dataset_generator = data_generator(dataset, self.config, shuffle=False,
                                           batch_size=self.config.BATCH_SIZE,
                                           output_names=self.keras_model.output_names)
        ...
        self.keras_model.predict_generator(
            dataset_generator,
            steps_per_epoch=self.config.STEPS_PER_EPOCH,
            max_queue_size=100,
            workers=workers,
