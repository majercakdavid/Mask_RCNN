...
class COCOeval:
    ...
    def __init__(self, cocoGt=None, cocoDt=None, iouType='segm'):
        ...
        self._gts_img = defaultdict(list)
        self._dts_img = defaultdict(list)
    ...
    def _prepare(self):
        ...
        for gt in gts:
            ...
            self._gts_img[gt['image_id']].append(gt)
        for dt in dts:
            self._dts[dt['image_id'], dt['category_id']].append(dt)
            self._dts_img[dt['image_id']].append(dt)
    ...
    def get_cm(self):
        ...
        for imgId in p.imgIds:
            labels, preds = self.computePredsLabelsForImage(imgId)
            self.labels_preds['labels'].extend(labels)
            self.labels_preds['preds'].extend(preds)
            self.labels_preds['imageIds'].extend([imgId]*len(preds))

        return confusion_matrix(self.labels_preds['labels'], self.labels_preds['preds'])
    ...
    def evaluate(self):
        ...
        evaluateCats = self.evaluateCats
        maxDet = p.maxDets[-1]
        self.evalCat = [evaluateCats(catId)
                        for catId in catIds
                        ]
...
class Params:
    ...
    def setDetParams(self):
        ...
        self.iouThrs = np.linspace(.5, 0.95, np.int(
            np.round((0.95 - .5) / .05)) + 1, endpoint=True)
        self.recThrs = np.linspace(.0, 1.00, np.int(
